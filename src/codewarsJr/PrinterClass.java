...
public class PrinterClass {
	...
	public String printerError(String s) {
		...
		System.out.println("");
		String[] arrayNZ = { "n", "o", "p", "q", "r", "s", "t", "u", "v", "x", "w", "y", "z" };
		String[] arraySplit = null;
		arraySplit = s.split("");
		int countTotal = arraySplit.length;
		int countNZ = 0;

		for (int i = 0; i < arraySplit.length; i++) {
			for (int j = 0; j < arrayNZ.length; j++) {
				if (arraySplit[i].equals(arrayNZ[j])) {
					countNZ++;
				}
			}
		}

		String countErr = (String.valueOf(countNZ) + "/" + String.valueOf(countTotal));
